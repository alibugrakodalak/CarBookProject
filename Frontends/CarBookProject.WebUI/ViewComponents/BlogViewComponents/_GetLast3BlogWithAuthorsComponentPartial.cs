...
namespace CarBookProject.WebUI.ViewComponents.BlogViewComponents
{
	public class _GetLast3BlogWithAuthorsComponentPartial : ViewComponent
	{
		...
		public async Task<IViewComponentResult> InvokeAsync()
		{
			...
			var responseMessage = await client.GetAsync("https://localhost:7095/api/Blogs/GetLast3BlogWithAuthorsList");
			if (responseMessage.IsSuccessStatusCode)
			{
				var jsonData = await responseMessage.Content.ReadAsStringAsync();
				var values = JsonConvert.DeserializeObject<List<ResultGetLast3BlogWithAuthorDto>>(jsonData);
				return View(values);
			}
			return View();
		}
