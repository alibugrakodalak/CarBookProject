...
namespace CarBookProject.Application.Features.Mediator.Handlers.AppUserHandlers
{
	public class GetCheckAppUserQueryHandler : IRequestHandler<GetCheckAppUserQuery, GetCheckAppUserQueryResult>
	{
		...
		public async Task<GetCheckAppUserQueryResult> Handle(GetCheckAppUserQuery request, CancellationToken cancellationToken)
		{
			...
			var user = await _appUserRepository.GetFilterByAsync(x=>x.Username == request.Username && x.Password == request.Password);
			if(user == null)
			{
			...
			else
			{
				...
				values.Role			= (await _appRoleRepository.GetFilterByAsync(x => x.AppRoleId ==							user.AppUserId)).AppRoleName;
